...
assert.equal(structuredSnapshotIds.length, 35, "all 35 released events must bind structured facts");
assert.deepEqual(structuredFamilyCounts, { FOMC: 5, EMP: 7, CPI: 7, PCE: 8, GDP: 8 }, "structured fact coverage must match the five approved released-family baselines");
assert.deepEqual(new Set(structuredSnapshotIds), directSourceIds, "structured facts must cover the same 35 released IDs as direct official sources");
...
const result = {
  status: "PASS",
  inventory: { releaseEvents: ids.length, familyCounts, releasedAtCutoff: 35, scheduledAtCutoff: 23, observationRows: 36, releasedStructuredFacts: structuredSnapshotIds.length },
